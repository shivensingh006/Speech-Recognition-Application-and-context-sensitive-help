...
var defaultOptions = {
  ...
  defaultUrl: {
    value: 'AMT_PTD_TR_0014_13_1_EN.pdf',
  ...
  _createClass(PDFFindController, [{
    ...
    value: function _calculatePhraseMatch(query, pageIndex, pageContent, entireWord) {
      ...
      while (true) {
		...
		//In pdf point the header seaction when click on help menu.
		var urlval = window.location.href;
		var hashval = urlval.split('#')[1];
		var hashparam = hashval.split('=')[0];
		var val;
		var query2;
		//console.log(hashparam+'____'+z);
		if(hashparam == 'searchhelp' && z== 1){	
			var elems = document.getElementsByClassName('outlineItem');
			for (var i=0;i<elems.length;i+=1){
				val = elems[i].getElementsByTagName('a')[0].innerHTML.replace(/\s/g, '').toUpperCase();
				query2 = query.replace(/\s/g, '').toUpperCase();
			if(val.match(query2)){
				elems[i].scrollIntoView();
				elems[i].getElementsByTagName('a')[0].classList.add("activesection");
			}
			}
			matches.push(matchIdx);
		}else if(hashparam != 'searchhelp'){	
			matches.push(matchIdx);
		}
		z++;
      }
    ...
    value: function setHash(hash) {
      ...
      if (hash.includes('=')) {
        ...
        if ('search' in params) {
		  ...
		  document.getElementById('findInput').value=params['search'];
		  document.getElementById('viewFind').click();
		  document.getElementById("findEntireWord").click();
		  document.getElementById("findEntireWord").checked = true;
        }
		
		 if ('searchhelp' in params) {
          this.eventBus.dispatch('findfromurlhash', {
            source: this,
            query: params['searchhelp'].replace(/"/g, ' '),
            phraseSearch: params['phrase'] === 'true'
          });
		  document.getElementById('findInput').value=params['searchhelp'];
		  document.getElementById('viewFind').click();
		  document.getElementById("findMatchCase").click();
		  document.getElementById("findMatchCase").checked = true;
		  document.getElementById('findbar').style.display = "none";
		  document.getElementById('viewFind').onclick = function(){
	   ...
	   var elems = document.getElementsByClassName("outlineItem");
		for (var i = 0; i < elems.length; i++){
			elems[i].onclick = function (){
				...
				while(e1){
					if(e1.tagName === 'DIV'){
						e1.classList.remove('activesectionsub');
					}
					e1 = e1.nextSibling;
				}
				this.classList.add('activesectionsub');
			}
